...
def connect_between_songs_by_dtw_only(song1: Song, song2: Song, file_path: str, song_builder: Song = None,
    ...
    dtw_cost_matrix, wp = librosa.sequence.dtw(suffix_chroma_stft, prefix_chroma_stft,
                                               subseq=True)  # wp is the Warping path
...
def find_silent_intervals_of_partial(song: Song, threshold=0, min_time_interval_length=2, suffix=False):
    indices = np.where(song.suffix_vocals_energy < threshold)[0] if suffix else \
    np.where(song.prefix_vocals_energy < threshold)[0]
...
def find_silent_intervals_of_suffix_and_prefix(song: Song, threshold=0, min_time_interval_length=2):
    ...
    if len(suffix_scilent_intervals) == 0:
        suffix_scilent_intervals = [
            (song.partial_audio_time_in_sec - MIN_SIZE_OF_TIME_INTERVAL, song.partial_audio_time_in_sec)]
...
def dtw_over_songs_intervals_post_spleeter(song1: Song, song2: Song, song1_suffix_scilent_intervals,
                                           song2_prefix_scilent_intervals):
    min_value = np.inf
    ...
    for i in range(len(song1_suffix_scilent_intervals)):
        for j in range(len(song2_prefix_scilent_intervals)):
            chroma_suffix_song1 = song1.suffix_accompaniment.get_partial_chroma_stft(
                start_sec=song1_suffix_scilent_intervals[i][0], \
                end_sec=song1_suffix_scilent_intervals[i][1])
            chroma_prefix_song2 = song2.prefix_accompaniment.get_partial_chroma_stft(
                start_sec=song2_prefix_scilent_intervals[j][0], \
                end_sec=song2_prefix_scilent_intervals[j][1])
            dtw_cost_matrix, wp = librosa.sequence.dtw(chroma_suffix_song1, chroma_prefix_song2,
                                                       subseq=True)  # wp is the Warping path
            ...
            for idx in sorted_indeces:
                ...
                first_audio = song1.get_partial_audio(
                    end_sec=len(song1.audio) / song1.sr - song1.partial_audio_time_in_sec +
                            song1_suffix_scilent_intervals[min_i][0] + suffix_cut_audio_index / song1.sr)
                if song1.get_audio_energy_array(get_partial_audio(first_audio, song1.sr, start_sec=-2))[
                    0].mean() > NOISE_THRESHOLD_FOR_TRANSFORMATION:
                    break
...
def concat_between_songs_post_spleeter(song1: Song, song2: Song, file_path: str, song_builder: Song = None):
    ...
    suffix_cut_audio_index = suffix_cut_frame * 512
    first_song = song_builder if song_builder is not None else song1
    first = first_song.get_partial_audio(end_sec=len(first_song.audio) / song1.sr - song1.partial_audio_time_in_sec +
                                                 song1_suffix_scilent_intervals[min_i][
                                                     0] + suffix_cut_audio_index / song1.sr)
...
if __name__ == '__main__':
    ...
    for song_name in song_names:
        ...
        song = Song(os.path.join(SONGS_LIST_DIR, song_name), seperator=sep, remove_zero_amp=True)
    ...
    outpath = "numpy_arrays\\dtw_outputs"
    ...
    for i in range(len(songs) - 1):
        ...
        new_song, best_tuple_cuts_in_sec = connect_between_songs_by_dtw_only(songs[i], songs[i + 1],
                                                                             file_path=result_fp, song_builder=new_song)
    ...
    full_playlist_audio = new_song.audio
    np.save(os.path.join(outpath, f'baseline_playlister_playlist_numpy.npy'), full_playlist_audio)
